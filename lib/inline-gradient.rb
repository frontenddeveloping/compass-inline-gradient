...
module InlineGradient
    ...
    class << self
        ...
        def inline_gradient(type = DEFAULT_TYPE, width = DEFAULT_WIDTH, height = DEFAULT_HEIGHT, angle = DEFAULT_ANGLE , *color_stops)
            ...
            if type == "linear"
                inline_linear_gradient(width, height, angle, color_stops)
            else type == "radial"
                inline_radial_gradient(width, height, angle, color_stops)
            end
        ...
        def inline_linear_gradient(width, height, angle, color_stops)
            ...
            color_stops.each do |color_stop|
                ...
                image_list.new_image(current_distance - prev_distance, height, fill);
        ...
        def distance2px (distance, width)
            ...
            if distance.end_with?('%')
                distance = ( distance.to_f / 100 ) * width
            end
            distance.to_i
        end
        def side2angle (side)
